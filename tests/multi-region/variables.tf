...
variable "resource_group_name_primary" {
  ...
  default     = "tfe-primary-prereqs-rg"
}

variable "resource_group_name_secondary" {
  type        = string
  description = "Name of Resource Group to create."
  default     = "tfe-secondary-prereqs-rg"
}
...
variable "friendly_name_prefix_primary" {
  type        = string
  description = "Friendly name prefix for unique Azure resource naming across deployments."

  validation {
    condition     = can(regex("^[[:alnum:]]+$", var.friendly_name_prefix_primary)) && length(var.friendly_name_prefix_primary) < 13
    error_message = "Must only contain alphanumeric characters and be less than 13 characters."
  }
}

variable "friendly_name_prefix_secondary" {
  type        = string
  description = "Friendly name prefix for unique Azure resource naming across deployments."

  validation {
    condition     = can(regex("^[[:alnum:]]+$", var.friendly_name_prefix_secondary)) && length(var.friendly_name_prefix_secondary) < 13
    error_message = "Must only contain alphanumeric characters and be less than 13 characters."
  }
}
...
# VNet
#-------------------------------------------------------------------------
variable "vnet_name_primary" {
  type        = string
  description = "Name of VNET to create."
  default     = "tfe-vnet-primary"
}

variable "vnet_name_secondary" {
  type        = string
  description = "Name of VNET to create."
  default     = "tfe-vnet-secondary"
}
...
variable "vnet_cidr_secondary" {
  type        = list(string)
  description = "CIDR block address space for VNet."
  default     = ["10.1.0.0/16"]
}

variable "bastion_subnet_cidr_primary" {
  type        = string
  description = "CIDR block for bastion subnet."
  default     = "10.0.255.0/24"
}

variable "bastion_subnet_cidr_secondary" {
  type        = string
  description = "CIDR block for bastion subnet."
  default     = "10.1.0.0/24"
}
...
variable "vm_subnet_cidr_primary" {
  type        = string
  description = "CIDR block for TFE VM subnet."
  default     = "10.0.2.0/24"
}

variable "vm_subnet_cidr_secondary" {
  type        = string
  description = "CIDR block for TFE VM subnet."
  default     = "10.1.2.0/24"
}

variable "db_subnet_cidr_primary" {
  type        = string
  description = "CIDR block for TFE database subnet."
  default     = "10.0.3.0/24"
}

variable "db_subnet_cidr_secondary" {
  type        = string
  description = "CIDR block for TFE database subnet."
  default     = "10.1.3.0/24"
}
